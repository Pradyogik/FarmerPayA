...
const OPENAI_API_KEY = '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';
const API_URL = 'https://api.openai.com/v1/chat/completions';
const MODEL = 'gpt-4o-mini';

const BHASHINI_URL = 'https://dhruva-api.bhashini.gov.in/services/inference/pipeline';
const BHASHINI_AUTH = 'G1PL5w8C4R-YZHUK3JuNGbNZzLPwaNrhxjmGOtfuUQahFpdyfURwbsgyVYNx5vc-'; // from your cURL
...
const ChatScreen: React.FC<Props> = ({ route }) => {
  ...
  const pushMessage = (role: Role, text: string) => {
    setMessages(prev => [
      ...prev,
      { id: `${Date.now()}-${Math.random()}`, role, text, createdAt: Date.now(), reaction: null },
    ]);
  };
  ...
  const askAssistant = async (userText: string) => {
    try {
      ...
      console.error(e);
      pushMessage('assistant', '❗ There was an error fetching the response. Please try again.');
    } finally {
      setIsThinking(false);
    }
  };
  ...
  const transcribeWithBhashini = async (base64Audio: string) => {
    try {
      ...
      const asr =
        json?.pipelineResponse?.find?.((t: any) => t?.taskType?.toLowerCase() === 'asr') ||
        json?.pipelineResponse?.[0];

      const out = asr?.output?.[0];
      const transcript =
        out?.source || out?.transcript || out?.text || json?.output?.[0]?.source || '';
  ...
  const handleReaction = (id: string, reaction: 'like' | 'dislike') => {
    ...
    showToast(reaction === 'like' ? 'Thanks for the feedback!' : "We’ll improve this");
  };
  ...
  const AssistantActions = ({
    ...
    const bump = (v: Animated.Value) =>
      Animated.sequence([
        Animated.timing(v, { toValue: 1.2, duration: 120, easing: Easing.out(Easing.quad), useNativeDriver: true }),
    ...
    return (
      <View style={styles.actions}>
        ...
        <TouchableOpacity
          onPress={() => { onLike(); bump(likeScale); }}
          style={styles.actionBtn}
          hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}
        >
          <Animated.View style={{ transform: [{ scale: likeScale }] }}>
            <LikeIcon size={18} color={reaction === 'like' ? '#4CAF50' : '#7A7A7A'} strokeWidth={1.6} />
          </Animated.View>
        </TouchableOpacity>

        <TouchableOpacity
          onPress={() => { onDislike(); bump(dislikeScale); }}
          style={styles.actionBtn}
          hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}
        >
          <Animated.View style={{ transform: [{ scale: dislikeScale }] }}>
            <DislikeIcon size={18} color={reaction === 'dislike' ? '#E53935' : '#7A7A7A'} strokeWidth={1.6} />
          </Animated.View>
        </TouchableOpacity>
      </View>
    );
              ...
              <View style={styles.voiceWave}>
                {barValues.map((v, i) => (
                  <Animated.View
                    ...
                    style={[
                      styles.voiceBar,
                      { height: v.interpolate({ inputRange: [0, 1], outputRange: [8, 28] }) },
                    ]}
                  />
                ))}
              </View>
...
const styles = StyleSheet.create({
  ...
  // Thinking
  thinkingRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 3 },
  dot: { width: 6, height: 6, borderRadius: 3, backgroundColor: '#999', marginRight: 6 },
  ...
  inputGradient: {
    flex: 1, padding: 1, borderRadius: 40, minHeight: 56,
    justifyContent: 'center', overflow: 'hidden',
  },
  inputInner: {
    backgroundColor: 'white', borderRadius: 39, minHeight: 56,
    paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center',
  },
  textInput: { flex: 1, fontSize: 16, color: '#1F077A', paddingVertical: 8 },
  sendBtnSolid: {
    width: 34, height: 34, borderRadius: 17, backgroundColor: '#6929C4',
    justifyContent: 'center', alignItems: 'center', marginLeft: 6,
    shadowColor: '#6929C4', shadowOpacity: 0.25, shadowRadius: 6,
    shadowOffset: { width: 0, height: 2 }, elevation: 4,
  },
  micBtnSolid: {
    width: 32, height: 32, borderRadius: 50, backgroundColor: '#6929C4',
    justifyContent: 'center', alignItems: 'center', marginLeft: 6,
    shadowColor: '#6929C4', shadowOpacity: 0.25, shadowRadius: 6,
    shadowOffset: { width: 0, height: 2 }, elevation: 4,
  },
  ...
  voiceWave: {
    flexDirection: 'row', alignItems: 'flex-end', height: 30,
    gap: 3, flex: 1, marginRight: 12,
  },
  voiceBar: {
    width: 4, backgroundColor: '#6929C4', borderRadius: 2,
  },
  timerText: { fontSize: 14, color: '#1F077A', marginRight: 12, fontWeight: '600' },
  stopBtn: {
    width: 36, height: 36, borderRadius: 18, backgroundColor: '#E53935',
    justifyContent: 'center', alignItems: 'center',
  },
  stopSquare: { width: 14, height: 14, backgroundColor: 'white', borderRadius: 3 },

  // Toast
  toast: {
    position: 'absolute', bottom: 96, alignSelf: 'center',
    backgroundColor: '#1F077A', borderRadius: 16,
...
const markdownStyles = {
  ...
  heading1: { fontSize: 20, fontWeight: '700', marginBottom: 6, color: '#171717' },
  heading2: { fontSize: 18, fontWeight: '700', marginBottom: 6, color: '#171717' },
  heading3: { fontSize: 16, fontWeight: '700', marginBottom: 6, color: '#171717' },
  code_block: {
    backgroundColor: '#F1F3F5', padding: 8, borderRadius: 8,
